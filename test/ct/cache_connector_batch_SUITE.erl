...
strong_fetch_batch_overlap_expire(_) ->
  ...
  lists:foldl(fun(I, Acc) ->
   ...
   timer:sleep(1000),
{ok, Values2} = cache_connector:fetch_batch(#{
             type => strong,
             cmd_fn => CmdFn,
             ttl => 2000,
             keys => Keys2,
...
strong_fetch_batch_error(_) ->
  ...
  {error, not_found} = cache_connector:fetch_batch(#{
             type => strong,
             cmd_fn => CmdFn,
             ttl => 60000,
             keys => Keys,
...
get_value(Prefix, Keys) ->
  [<<Prefix/binary, (erlang:integer_to_binary(I))/binary>> || I <- lists:seq(1, erlang:length(Keys))].
get_batch_data(N) ->
...
get_keys(I, {Keys, Values1, Values2, Values3}) ->
  ...
  KeyPrefix = erlang:iolist_to_binary(cool_tools:uuid_v1_string()),
